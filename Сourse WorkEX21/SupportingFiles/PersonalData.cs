...
namespace Сourse_WorkEX21.SupportingFiles.Methods
{
    public class PersonalData
    {
        ...
        public bool getPersonalData(string login, string password)
        {
            try
            {
                string query = "SELECT * FROM usersregist ur LEFT JOIN userpersonaldata upd ON upd.RegistUserId = ur.Id " +
                    "WHERE ur.Login = @Login AND ur.Password = @Password LIMIT 1";
                ...
                using (MySqlCommand command = new MySqlCommand(query, database.getConnection()))
                {
                    ...
                    using (MySqlDataReader reader = command.ExecuteReader())
                    {
                        if (reader.HasRows)
                        {
                            ...
                            DataRegistration = reader.IsDBNull(reader.GetOrdinal("DataRegistration")) ? string.Empty : DateTime.Parse
                                (reader["DataRegistration"].ToString()).ToString("yyyy-MM-dd");
                            return true;
                        }
                    }
                    return false;
        ...
        {
            if (getPersonalData(login, password))
            {
        ...
        public Image LoadProfileImage()
        {
            ...
            {
                MessageBox.Show($"Помилка при завантаженні фото профіля: {ex.Message}", "Помилка", 
        ...
        private Image LoadDefaultImage()
        {
            try
            {
                return Properties.Resources.UserIcon;
            }
            catch (Exception ex)
            {
                MessageBox.Show($"Помилка при завантаженні базового фото профіля: {ex.Message}", "Помилка", 
                    MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        ...
        public string GetUserRole(string login, string password)
        {
            try
            {
                string query = "SELECT Status FROM usersregist ur LEFT JOIN userpersonaldata upd ON upd.RegistUserId = ur.Id " +
                    "WHERE ur.Login = @Login AND ur.Password = @Password LIMIT 1";
